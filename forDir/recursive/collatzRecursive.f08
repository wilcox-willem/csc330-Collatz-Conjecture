...
contains
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!
    recursive function collatzNumofSequences(input, step) result(resultStep)
        integer(kind=8), intent(in) :: input, step
        integer(kind=8) :: resultStep

        if (input < 2) then
            resultStep = step
        elseif (mod(input, 2) == 0) then
            resultStep = collatzNumofSequences(input / 2, step + 1)
        else
            resultStep = collatzNumofSequences(input * 3 + 1, step + 1)
        end if
    ...
    subroutine collatzNumofSequencesPrintTopTen(lowerLimit, upperLimit)
        ...
        integer(kind=8) :: currMin, i, currentresult, cycles, startStep
        ...
        do i = lowerLimit, upperLimit
            currentresult = collatzNumofSequences(i, startStep)
...
program CollatzRecursive
    use CollatzModule

    integer(kind=8) :: lowerLimit, upperLimit, startStep, inputLimit, absoluteLimit, absoluteOdd
    integer :: num_args
    ...
    if (lowerLimit > 0 .and. upperLimit == 0) then
        write(*, '(2I20)') lowerLimit, collatzNumofSequences(lowerLimit, startStep)
