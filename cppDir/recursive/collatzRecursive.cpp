...
// main functions
uint64_t collatzNumofSequencesRecursive(uint64_t input, uint64_t numofSeq);
...
int main(int argc, char* argv[]) {
    ...
    if ((lowerLimit > 0) && (upperLimit == 0)) {
      uint64_t result = collatzNumofSequencesRecursive(lowerLimit, 0);
      printf("%20d%20d\n", lowerLimit, result);
...
uint64_t collatzNumofSequencesRecursive(uint64_t input, uint64_t numofSeq) {
    ...
    if (input % 2 == 0) {
        return collatzNumofSequencesRecursive(input / 2, numofSeq + 1);
    } else {
        return collatzNumofSequencesRecursive((input * 3) + 1, numofSeq + 1);
    }
}
...
void collatzNumofSequencesPrintTopTen(uint64_t lowerLimit, uint64_t upperLimit) {
  ...
  for (uint64_t i = lowerLimit; i <= upperLimit; i++) {
    uint64_t result = collatzNumofSequencesRecursive(i, 0);
    if (result > currMin) {
